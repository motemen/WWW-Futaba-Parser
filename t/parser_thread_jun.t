...
my $thread = WWW::Futaba::Parser::Thread->parse(fake_http 'http://jun.2chan.net/b/res/13042338.htm');
isa_ok $thread, 'WWW::Futaba::Parser::Result::Thread';

is $thread->body, "そろそろマジレスフォルダ\n捨てようと思うんだけど、\nもったいないかな？どう？", 'thread body';
is $thread->datetime . '', '2010-02-27T08:22:14', 'thread datetime';
...
subtest simple_post => sub {
    ...
    is $posts[0]->datetime . '', '2010-02-27T08:26:00';
...
subtest post_with_image => sub {
    ...
    is $posts[1]->datetime . '', '2010-02-27T08:30:13';
...
subtest post_with_mail => sub {
    ...
    is $posts[2]->datetime . '', '2010-02-27T08:34:08';
    is $posts[2]->head->{no}, '13042361';
    is $posts[2]->head->{mail}, 'sage';
